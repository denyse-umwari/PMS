...
def read_last_unpaid_entry(plate):
    ...
    try:
        with open('plates_log.csv', 'r') as file:
            ...
            entries = [row for row in reader if row['Plate Number']
                       == plate and row['Payment Status'] == '0']
            if not entries:
                return None
        ...
        with open('plates_log.csv', 'w', newline='') as file:
            ...
            writer.writerow(['Plate Number', 'Payment Status',
        ...
        if ser.in_waiting > 0:
            ...
            if line.startswith("DATA:"):
                ...
                if cash <= 200:
                    ...
                    print(
                        f"[{get_timestamp()}] Balance ({cash} units) must be > 200 units.\n")
                    continue
                ...
                if last_entry is None:
                    ...
                    print(
                        f"[{get_timestamp()}] No unpaid entry for plate {plate}. Assuming 0 hours.\n")
                    hours = 0
                else:
                ...
                if charge > cash:
                    ...
                    print(
                        f"[{get_timestamp()}] Charge ({charge} units) exceeds balance ({cash} units).\n")
                    continue
                ...
                if response == "DONE":
                    ...
                    print(
                        f"[{get_timestamp()}] Unexpected response: {response}\n")
