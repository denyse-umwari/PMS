...
void loop() {
  ...
  if (!uidMatch) {
    Serial.println(
        "This card's UID does not match 5B ED 5F 3B. Aborting write.");
    rfid.PICC_HaltA();
  ...
  for (byte i = 0; i < size; i++) {
    buffer[i] =
        (i < licensePlate.length()) ? licensePlate[i] : ' '; // Pad with spaces
  }
  ...
  for (byte i = 0; i < size; i++) {
    buffer[i] =
        (i < cashAmount.length()) ? cashAmount[i] : ' '; // Pad with spaces
  }
